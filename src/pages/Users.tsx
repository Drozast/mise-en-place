  ...
  if (user?.role !== 'chef') {
    return (
      <div className="text-center py-12">
        <p className="text-xl text-dark-300">No tienes permisos para acceder a esta página</p>
      </div>
    );
  }
      ...
      <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
        <div className="space-y-3">
          {users.map((u) => (
            <div
              key={u.id}
              className={`flex items-center gap-4 p-4 bg-dark-900 border rounded-lg transition-colors ${
                u.active ? 'border-dark-700' : 'border-dark-800 opacity-60'
              }`}
            >
